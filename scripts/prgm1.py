...
def preprocessing(dataset):
    ...
    for i in cat_data[:, :-1]:
        ...
        for i in cat_data[:, :-1]:
            ...
            if "Neutered" in i[5]:
                cat_data[counter, 10] = 0
            counter += 1
        # dog_data = np.delete(dog_data,5,1)
        # cat_data = np.delete(cat_data,5,1)
    ...
    # is name present :1 else 0
        ...
        # %%
        indices = {"Herding":14,'Hound':15,'Mix':16,'Non-Sporting':17,
                   'Pit Bull':18,'Sporting':19,'Terrier':20,'Toy':21,
                   'Unknown':22,'Working':23}
